...
class Crawler {
  ...
  public async getTargetPrecedentsLength(type: 'issue' | 'recent') {
    ...
    const supremeCourtBtn = await this.page.$(
      '#groupList>li.last ul>li:nth-child(1) a'
    );
    if (supremeCourtBtn) {
      const precedentLength = await supremeCourtBtn.evaluate<EvaluateFn>(
        (elem) => elem.innerText.replace(/(대법원\(|\))/g, '')
      );
  ...
  private async setScrapTarget(type: 'issue' | 'recent') {
    ...
    await this.page.waitForTimeout(2000);
    if (type === 'recent') await this.setIssueTap();
  ...
  private async setViewLength(precedentLength: 20 | 40 | 80) {
    ...
    await this.page.evaluate(async (targetChildNum) => {
      const pagingSelect: HTMLOptionElement | null = document.querySelector(
        `.select_2.ml_4 option:nth-child(${targetChildNum})`
      );
      if (pagingSelect) pagingSelect.selected = true;
    }, targetChildNum);
  ...
  private async setSupremeCourtView() {
    ...
    if (supremeCourtBtn) await supremeCourtBtn.click();
    await this.page.waitForTimeout(3000);
  }
  ...
  private async getTargetPageCount() {
    ...
    if (pagingElem) {
      let pageCounts = await pagingElem.evaluate<EvaluateFn>(
        (elem) => elem.innerText.trim().split('/')[1]
      );
  ...
  private async scrapPrecedentSections(sections: ElementHandle[]) {
    const unfilteredResult = await Promise.all(
      sections.map((section) => this.scrapPrecedentSection(section))
    );
    return unfilteredResult.filter((precedent) =>
      Object.values(precedent).every((value) => value)
    );
  }
  ...
  private async scrapPrecedentNameAndType(section: ElementHandle) {
    ...
    if (titleElem) {
      ...
      type = filterPrecedentType(
        classifyString !== null ? classifyString[0].replace(/\d/g, '') : ''
      );
    }
    return { name, type };
  }
  ...
  private async scrapPrecedentContent(section: ElementHandle) {
    await section.evaluate((elem) => {
      const contentRevealButton: HTMLElement | null = elem.querySelector(
        'td:nth-child(2)>dl>dt>a:nth-child(2)'
      );
      if (contentRevealButton) contentRevealButton.click();
    });

    await this.page.waitForTimeout(5000);

    let content = '';
    const contentElem = await section.$(
      'td:nth-child(2)>dl>dd:nth-child(2)>dl>dd'
    );
    const contentText = await contentElem?.evaluate<EvaluateFn>(
      (elem: HTMLElement) => elem.innerHTML
    );
